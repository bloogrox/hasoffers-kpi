...
def get_pacc():
    ...
    out = (seq(res.data['data'])
           ...
           .filter(lambda row: int(row['gross_clicks']) >= get_offer_min_clicks(row['offer_id']))
           ...
           .map(lambda row: assoc(row, 'value', (row['profit'] - (row['gross_clicks'] * settings.CLICK_COST))))
           .to_list())
